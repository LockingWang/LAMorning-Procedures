CREATE OR ALTER PROCEDURE [dbo].[sp_UpdateOrderPaymentStatus] 
...
BEGIN 
    ...
    BEGIN TRY 
        ...
        BEGIN 
            ...
            -- 將訂單資料寫入 P_ChecksTemp_Web 暫存表 
            INSERT INTO P_ChecksTemp_Web ( 
                GID, 
                EnterpriseID, 
                SHOPID, 
                BatchID, 
            ...
            VALUES ( 
                ...
                @ShopId,                -- SHOPID 
                @PlatformOrderId,       -- BatchID 
                @OrderId,               -- OrderID 
                @ConvertedAction,       -- CheckID (使用轉換後的值)
                @ConvertedAction,       -- CheckName (使用轉換後的值)
                @Amount,                -- Total 
                @ConvertedAction,       -- Remark (使用轉換後的值)
                GETDATE(),              -- LastModify 
                NULL,                   -- DeskID 
                NULL,                   -- ItemsData 
                NULL,                   -- PCommand 
