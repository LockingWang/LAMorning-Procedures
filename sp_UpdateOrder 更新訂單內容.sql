...
BEGIN      
	...
	BEGIN TRY      
		BEGIN TRANSACTION      
			...
			-- 檢查訂單是否已存在
			DECLARE @ExistingOrderCount INT = 0;
			SELECT @ExistingOrderCount = COUNT(*) 
			...
			BEGIN
				...
				SET LastModify = @now,
                    OrderStatus = 0,
					-- 以下欄位與原有數值相加
					[Count] = ISNULL([Count], 0) + ISNULL((SELECT CAST(JSON_VALUE(@OrderJson, '$.Count') AS FLOAT)), 0),
					OrderFoodCount = ISNULL(OrderFoodCount, 0) + ISNULL((SELECT CAST(JSON_VALUE(@OrderJson, '$.OrderFoodCount') AS DECIMAL)), 0),
					Total = ISNULL(Total, 0) + ISNULL((SELECT CAST(JSON_VALUE(@OrderJson, '$.Total') AS FLOAT)), 0),
					PayTotal = ISNULL(PayTotal, 0) + ISNULL((SELECT CAST(JSON_VALUE(@OrderJson, '$.PayTotal') AS FLOAT)), 0),
					ServiceTotal = ISNULL(ServiceTotal, 0) + ISNULL((SELECT CAST(JSON_VALUE(@OrderJson, '$.ServiceTotal') AS FLOAT)), 0),
					ServicePercent = ISNULL(ServicePercent, 0) + ISNULL((SELECT CAST(JSON_VALUE(@OrderJson, '$.ServicePercent') AS FLOAT)), 0),
					AgioDiscount = ISNULL(AgioDiscount, 0) + ISNULL((SELECT CAST(JSON_VALUE(@OrderJson, '$.AgioDiscount') AS DECIMAL)), 0),
					AgioCost = ISNULL(AgioCost, 0) + ISNULL((SELECT CAST(JSON_VALUE(@OrderJson, '$.AgioCost') AS DECIMAL)), 0),
					SubmitTimes = ISNULL(SubmitTimes, 0) + 1
				OUTPUT INSERTED.*
				WHERE ID = @OrderID;
			END
			ELSE
			BEGIN
				...
				INSERT INTO P_OrdersTemp_Web (EnterpriseID,ShopID,OPERATOR,OrderStatus,OrderNo2,LastModify,ID,DeskID,Machine,VendNo,SaleTime,Man,Woman,Child,Baby,      
					[Count],Total,PayTotal,ServicePercent,ServiceTotal,TendTotal,InvoiceTotal,VIPNo,VIPName,SaleType,--LastModify,      
				...
				select @EnterpriseID,@ShopID,@Operator,0,@EnterpriseID + MeTime + @SerialNum,@now,ID,DeskID,Machine,VendNo,SaleTime,Man,Woman,Child,Baby,      
					[Count],Total,PayTotal,ServicePercent,ServiceTotal,TendTotal,InvoiceTotal,@MemberNo,@MemberName,SaleType,      
					MeTime,VIPTel,AgioDiscount,AgioCost,AgioPercent,CardID,WechatOpenID,OrderFoodCount,PayStatus,      
			...
			SELECT @EnterpriseID,@OrderID,@Operator,@ShopID,@now,ID,FoodID,MainID,KindID,Parent,[Add],[Count],Price,AddCost,Total,FoodName,      
				ISNULL(Taste, '') AS Taste,InputTime,ServCost,Discount,TotalDiscount,Special,Special1,ISNULL(Memo, '') AS Memo,ChangePrice,BatchID,
				-- OrderIndex 計算方式：SubmitTimes * 10000 + OrderIndex (方便確認是第幾筆訂單以及順序)
				(ISNULL((SELECT ISNULL(SubmitTimes, 0) FROM P_OrdersTemp_Web WHERE ID = @OrderID AND ShopID = @ShopID AND EnterpriseID = @EnterpriseID), 1) * 10000) + ISNULL(OrderIndex, 0) AS OrderIndex      
