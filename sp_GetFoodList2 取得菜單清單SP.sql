CREATE or ALTER PROCEDURE sp_GetFoodList2 
...
SELECT 
    ...
    CAST(CASE WHEN F.stop = 1 THEN 1 ELSE 0 END AS Bit) AS IsSoldOut  -- 是否停售 
FROM P_FoodMould_Shop FMS 
    ...
    JOIN P_FoodMould FM ON FM.EnterPriseID = @enterpriseId AND FM.MouldCode = FMS.MouldCode 
    -- 關聯食品小分類 
