	...
	BEGIN TRY  
		BEGIN TRANSACTION  
			...
			INSERT INTO P_OrdersTemp_Web (EnterpriseID,ShopID,OPERATOR,OrderStatus,OrderNo2,LastModify,ID,DeskID,Machine,VendNo,SaleTime,Man,Woman,Child,Baby,  
					Total,PayTotal,ServicePercent,TendTotal,InvoiceTotal,VIPNo,VIPName,SaleType,--LastModify,  
					...
					ReceiptOP,TakeWayOP2,CarrierId,PrintStatus,VipAddrHead)  
			OUTPUT INSERTED.*  
			select @EnterpriseID,@ShopID,@Operator,0,@EnterpriseID + MeTime + @SerialNum,GETDATE(),ID,DeskID,Machine,VendNo,SaleTime,Man,Woman,Child,Baby,  
					Total,PayTotal,ServicePercent,TendTotal,InvoiceTotal,@MemberNo,@MemberName,SaleType,  
					MeTime,VIPTel,AgioDiscount,CardID,WechatOpenID,OrderFoodCount,PayStatus,  
					...
					ReceiptOP,TakeWayOP2,CarrierId,PrintStatus,VipAddrHead FROM OPENJSON(@OrderJson)  
				WITH (  
					ID varchar (100) ,  
					DeskID varchar (100) ,  
					Machine varchar (24) ,  
